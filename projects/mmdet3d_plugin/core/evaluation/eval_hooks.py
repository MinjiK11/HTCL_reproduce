...
class OccDistEvalHook(BaseDistEvalHook):
    def __init__(self, *args, dynamic_intervals=None,  **kwargs):
        ...
        self.key_criterion_name = 'lidarseg_mean'
        self.key_score_best = -1
        self.best_epoch = -1
    ...
    def _do_evaluate(self, runner):
        ...
        from projects.mmdet3d_plugin.occupancy.apis.test import custom_multi_gpu_test # to solve circlur  import
        ...
        if runner.rank == 0:
            ...
            # saving best
            # real_key_score = key_score[self.key_criterion_name]
            # if real_key_score > self.key_score_best:
            #     self.key_score_best = real_key_score
            #     self.best_epoch = runner.epoch
            
            # runner.logger.info('best val performance for {} is {:.4f}, achieved in epoch {}'.format(
            #         self.key_criterion_name, self.key_score_best, self.best_epoch))
